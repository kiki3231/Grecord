...
public class GameBacklogController {
    ...
    @PutMapping("/{id}")
    public AjaxResult<?> update(@PathVariable Long id, @RequestBody GameBacklog backlog) {
        return AjaxResult.success("更新成功", gameBacklogService.updateBacklog(id, backlog));
    }

    @Operation(summary = "状态流转")
    @PutMapping("/{id}/status")
    public AjaxResult<?> updateStatus(@PathVariable Long id, @RequestBody Map<String, Integer> body) {
    ...
    public AjaxResult<?> checkin(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        Integer playTime = body.get("playTime") != null ? Integer.valueOf(body.get("playTime").toString()) : 0;
        String notes = body.get("notes") != null ? body.get("notes").toString() : null;
        return AjaxResult.success("打卡成功", gameBacklogService.checkinFromBacklog(id, playTime, notes));
    }
}
